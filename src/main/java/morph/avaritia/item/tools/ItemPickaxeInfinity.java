        ...
        if (stack.getTagCompound() != null) {
            if (stack.getTagCompound().getBoolean("hammer")) {
                if (!(victim instanceof EntityPlayer && AvaritiaEventHandler.isInfinite((EntityPlayer) victim))) {
                    int i = 10;
                    victim.addVelocity(-MathHelper.sin(player.rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F, 2.0D, MathHelper.cos(player.rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F);
                }
            }
        }
        return true;
