	...
	public void onCollideWithPlayer(EntityPlayer player)
    {
        if (!this.worldObj.isRemote)
        {
            ...
            for (int i=0; i<list.tagCount(); i++) {
	            ...
	            if (!cannotPickup()
	            		&& (owner == null || lifespan - this.getAge() <= 200 || owner.equals(player.getDisplayName()))
	            		&& (event.getResult() == Event.Result.ALLOW || player.inventory.addItemStackToInventory(itemstack)))
	            {
