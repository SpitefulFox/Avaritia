	...
	public BufferedImage modifyImage(BufferedImage image) {
		...
		if (anyOpaque && infinityImage != null) {
			...
			if (opaquecount > w * h * 0.3) {
				sx1 = MathHelper.floor(iw * 0.25);
				sx2 = MathHelper.floor(iw * 0.75);
				sy1 = MathHelper.floor(ih * 0.25);
				sy2 = MathHelper.floor(ih * 0.75);
			}
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				if (a != 0) {
					...
					r = Math.min(255, MathHelper.floor(rgb.getRed(inf) * l * brighten));
					g = Math.min(255, MathHelper.floor(rgb.getGreen(inf) * l * brighten));
					b = Math.min(255, MathHelper.floor(rgb.getBlue(inf) * l * brighten));
					for (i = 0; i < frames; i++) {
						frac = 0.125 * i;
						fr = MathHelper.floor((1 - frac) * r + frac * 255);
						fg = MathHelper.floor((1 - frac) * g + frac * 255);
						fb = MathHelper.floor((1 - frac) * b + frac * 255);
